...
exports.create = function create(parent) {
    ...
    return {
        ...
        resolve: function resolve(data, callback) {
            this._usedHandler = false;
            this._resolve(data, callback);
        },

        _resolve: function _resolve(data, callback) {
            ...
            if (thing.isObject(data)) {
                ...
                if (thing.isArray(data)) {
                    tasks = data.map(function (val) {
                        return _resolve.bind(self, val);
                    });
                } else {
                    tasks = {};
                    Object.keys(data).forEach(function (key) {
                        tasks[key] = _resolve.bind(self, data[key]);
                    });
                }
